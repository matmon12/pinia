<template>
  <div class="movie">
    <img :src="getImgUrl(movie.id)" :alt="movie.title" class="movie-img" />
    <div>
      <div class="movie-name">{{ movie.title }} ({{ movie.weight }})</div>
      <span class="movie-overview">{{ movie.composition }}</span>
      <div class="movie-buttons" v-if="!isSearch">
        <button
          class="btn movie-buttons-watched"
          @click="movieStore.toggleWatched(movie.id)"
        >
          <span v-if="!movie.isHit">Watched</span>
...
const getImgUrl = (imageNameWithExtension) =>
  new URL(`../img/1-${imageNameWithExtension}.webp`, import.meta.url).href;
</script>
